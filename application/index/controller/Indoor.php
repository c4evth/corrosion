  ...
  public function dataShow($id = 1,$search = '', $name = '',$page = 1)
  {
    $modelData = new commonDcate();
		...
		if($search == ''){
			...
			$getSearchCate = json_decode($modelData -> getIndoordataIdByName($name,1));
			$cidArr = $this -> changeArr($getSearchCate,'id');
			// 开始模糊查询
			$searchResult = json_decode($modelData -> getIndoorAllBySearch($search,1,$cidArr,$start,$length));
			$Data = $searchResult;
			$currSearch = $search;
			$this -> assign([
				'currName' => $name,
